...
## QA 2차 후속 (2026-07-10 밤, 피드백 6건)

1. 자주 여는 파일에 ✕ 제거 버튼 (기록 자체 삭제 — 최근 목록과 동일 동작). — ✅
2. **번역 UX 전면 교체**: 독의 번역 영역 삭제. 선택 번역(🌐)·페이지 번역 모두 **본문에 직접 반영**되고 **Ctrl+Z로 되돌리기**(문서당 20단계). 선택 번역은 원문 유일 매칭일 때만 치환, 실패 시 팝업 폴백. 반영 후 Ctrl+S면 번역본으로 저장됨.
3. 상단바 편집 버튼 옆에 **[페이지 번역] [대상 언어▾]** 배치 (네이티브 전용 표시). 페이지 번역은 1500자 줄 단위 분할 호출(URL 한계 회피), 진행 표시 …n/m.
4. 탭줄 스크롤바 숨김 + 탭이 많으면 축소(min 56px), 그래도 넘치면 휠 가로 스크롤. 오인 원인 제거.
5. 클립 메모에서 복사한 텍스트는 자동기록에 재기록되지 않음 (suppress 1회).
6. 대상 언어는 UI 언어와 독립 저장 유지 (`trLang`).

수동 확인 대기(네이티브): 선택 번역 반영/Ctrl+Z, 페이지 번역(긴 문서), 번역 후 저장, 클립 재기록 방지.
